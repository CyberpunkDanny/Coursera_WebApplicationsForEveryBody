<?php
			...
			&& isset($_POST["AddProfHeadline"]) && isset($_POST["AddProfSummary"]))
	{
		...
		$stmtPos->execute( array(
							':ph_year' => $_POST["AddProfYear"],
							':ph_desc' => $_POST["AddProfDesc"],
...
<head>
	...
	<script type="text/javascript">
		...
		function validateAddProfFields()
		{
			...
			for(i=1; i<=count; i++)
			{
				var year = document.getElementById("AddProfYear"+count).value;
				var desc = document.getElementById("AddProfDesc"+count).value;
				if(year.length<1 || desc.length<1)
				{
					console.log("Year or Description is unfilled");
					//document.getElementById("AddProfYear"+count).style.background = "#ff6666";
					//document.getElementById("AddProfDesc"+count).style.background = "#ff6666";
					document.getElementById("flashMsg").innerHTML = "All fields must be filled out";
					document.getElementById("flashMsg").style.color = "red";
					return false;
				}
		...
		$(document).ready(function(){
			...
			event.preventDefault();
				//alert("Inside posDiv");
				count++;
				console.log("Value of Count inside jQuery Func ::: "+count);
				var year = '<label>Year</label> <input type="text" name="AddProfYear'+count+'" id="AddProfYear'+count+'"/><br/>';
				var desc = '<label>Description</label> <br/><textarea name="AddProfDesc'+count+'" id="AddProfDesc'+count+'" rows="8" cols="82"></textarea><br/>';
				$('#posDiv').append(year, desc);
			});
		});
